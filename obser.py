...
def Obser0(self,  GR, phase, Weight, obs_scal_phase_sum_list, obs_eq_phase_sum_list, obs_geq_phase_sum_list, phase_sum, Weight_sum, obs_scal_len, obs_eq_len, obs_geq_len):
        N_FL = self.N_FL
        Ndim = self.Lx * self.Ly * self.Norbs * self.Nlayers
        K = hubbard_hopping_nf(self.Ham_t, self.Ham_tp, self.Ham_mu, self.Lx, self.Ly, self.N_FL, self.bc_x, self.bc_y, self.ladder)
        # Initialization
       ...
       # ZZ_r, ZXY_r, ZZ2XY_r = spin_spin_corr(GR, GRC, self.Lx, self.Ly)  # r-resolved arrays
        ...
        # ====== Initialize r-resolved spin observables ======
        #obs_eq_list = np.empty(3, dtype=object)        # 0=ZZ,1=ZXY,2=ZZ+2*ZXY
        #obs_eq_avg_list = np.empty(3, dtype=object)

        #for i in range(3):
        #    obs_eq_list[i] = np.zeros((self.Lx, self.Ly), dtype=np.float64)
        #    obs_eq_avg_list[i] = np.zeros((self.Lx, self.Ly), dtype=np.float64)

        # ====== Accumulate r-resolved observables ======
        #    obs_eq_list[0] += ZZ_r * phase
        #    obs_eq_list[1] += ZXY_r * phase
        #    obs_eq_list[2] += ZZ2XY_r * phase
        ...
        #density-density correlation: <n_i n_j> - <n_i><n_j>
        #DD_r = density_density_corr(GR, GRC, Lx, Ly)    
        # Pair-pair correlation: <c_i↑ c_i↓ c_j↓† c_j↑†>
        #pairing == 's',pairing == 'px',pairing == 'py',pairing == 'd' # 
        #P_r = form_factor(dx, dy, pairing)
        ...
        return obs_scal_avg_list, obs_eq_avg_list, obs_geq_avg_list, obs_scal_phase_sum_list, obs_eq_phase_sum_list, obs_geq_phase_sum_list, phase_sum, Weight_sum
