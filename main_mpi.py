...
def Main(self, This, tun_params):
    ...
    remainder = Nbin % size
    start_bin = rank * bins_per_rank + min(rank, remainder)
    ...
    if not self.Corr_all:
        ...
        obs_eq_avg_blist = np.zeros((obs_eq_len, local_nbin, Ndim,  Ndim), dtype = np.float32)
        
    obs_scal_avg_blist = np.zeros((obs_scal_len, local_nbin), dtype = np.float32)
    ...
    for bin in range(start_bin, end_bin):    
        ...
        if not self.Corr_all:
            size_x = self.Lx if self.bc_x == "periodic" else 2 * self.Lx - 1
            size_y = self.Ly if self.bc_y == "periodic" else 2 * self.Ly - 1
            #obs_eq_phase_sum_list = np.zeros((obs_eq_len, self.Lx, self.Ly), dtype = np.float32)
        ...
        if bin_count >= 0:
            ...
            Weight_avg_barray[local_bin_index]  = Weight_sum #/ Nsweep
            ...
            for j in range(obs_scal_len):
                obs_scal_avg_blist[j, local_bin_index] = obs_scal_avg_list[j]
                
            # List of all average equal time observables    
            for j in range(obs_eq_len):
                obs_eq_avg_blist[j, local_bin_index] = obs_eq_avg_list[j]
                
            # List of all average equal time observables    
            for j in range(obs_geq_len):
                obs_geq_avg_blist[j, local_bin_index] = obs_geq_avg_list[j]

                
            local_bin_index += 1

        t_total_elapsed += time.time() - t_bin_start
    ...
    # ================= MPI REDUCTION =================
    phase_all = comm.gather(phase_avg_barray, root=0)
    weight_all = comm.gather(Weight_avg_barray, root=0)
    obs_scal_all = comm.gather(obs_scal_avg_blist, root=0)
    obs_eq_all = comm.gather(obs_eq_avg_blist, root=0)
    obs_geq_all = comm.gather(obs_geq_avg_blist, root=0)

    if rank == 0:
        phase_avg_barray = np.concatenate(phase_all)
        Weight_avg_barray = np.concatenate(weight_all)
        obs_scal_avg_blist = np.concatenate(obs_scal_all, axis=1)
        obs_eq_avg_blist = np.concatenate(obs_eq_all, axis=1)
        obs_geq_avg_blist = np.concatenate(obs_geq_all, axis=1)
         
         
    if rank == 0:
        result = (K, Nbin_eff,obs_scal_avg_blist, obs_eq_avg_blist, obs_geq_avg_blist, phase_avg_barray,
                Weight_avg_barray, deltaG_max, deltaG_mean, deltaP_max, deltaP_mean, t_total_elapsed, acceptance_rate)
